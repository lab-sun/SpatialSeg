...
# def create_ring_regions(height, width, num_rings):
#     regions = []
#     total_area = height * width
#     avg_area = total_area / num_rings
...
#         ring_area = (num_rings - i - 1) * avg_area
#         target_height = 3 * np.sqrt(ring_area / 12)
#         target_width = 4 * np.sqrt(ring_area / 12)

#         # Calculate dimensions for the current ring
#         div_h = (current_bottom - current_top - target_height) / 2
#         div_w = (current_right - current_left - target_width) / 2

#         top = (current_top, current_top + div_h)
#         bottom = (current_bottom - div_h, current_bottom)
#         left = (current_left, current_left + div_w)
#         right = (current_right - div_w, current_right)
...
#         # Update boundaries for the next ring
#         current_top += div_h
#         current_left += div_w
#         current_bottom -= div_h
#         current_right -= div_w
...
class Model_RGBT_Mask2Former(LightningModule):
    def __init__(self, cfg):
        ...
        self.regions = create_ring_regions(480, 640, 9)
    ...
    def validation_epoch_end(self, validation_step_outputs):
        ...
        self.log('val/average_precision', precision.mean(), on_step=False, on_epoch=True, sync_dist=True, prog_bar=True)
        self.log('val/average_recall', recall.mean(), on_step=False, on_epoch=True, sync_dist=True, prog_bar=True)
        self.log('val/average_IoU', IoU.mean(), on_step=False, on_epoch=True, sync_dist=True, prog_bar=True)
        self.log('val/average_IoU_dist', IoU.mean(), on_step=False, on_epoch=True, sync_dist=False, prog_bar=True)
        
        for i in range(len(precision)):
            self.log(f"val(class)/precision_class_{self.label_list[i]}", precision[i], on_step=False, on_epoch=True, sync_dist=True)
            self.log(f"val(class)/recall_class_{self.label_list[i]}", recall[i], on_step=False, on_epoch=True, sync_dist=True)
            self.log(f'val(class)/Iou_{self.label_list[i]}', IoU[i], on_step=False, on_epoch=True, sync_dist=True)
    ...
    #     label_gt = labels.cpu().numpy().squeeze()
    #     prediction = logits.argmax(1).cpu().numpy().squeeze()

    #     for region in self.regions:
    #         top, left, bottom, right = region   

    #         top_region = label_gt[top[0]:top[1], left[0]:right[1]]
    #         bottom_region = label_gt[bottom[0]:bottom[1], left[0]:right[1]]
    #         left_region = label_gt[top[1]:bottom[0], left[0]:left[1]]
    #         right_region = label_gt[top[1]:bottom[0], right[0]:right[1]]

    #         pred_top_region = prediction[top[0]:top[1], left[0]:right[1]]
    #         pred_bottom_region = prediction[bottom[0]:bottom[1], left[0]:right[1]]
    #         pred_left_region = prediction[top[1]:bottom[0], left[0]:left[1]]
    #         pred_right_region = prediction[top[1]:bottom[0], right[0]:right[1]]

    #         label = np.concatenate([top_region.flatten(), bottom_region.flatten(), left_region.flatten(), right_region.flatten()])
    #         pred = np.concatenate([pred_top_region.flatten(), pred_bottom_region.flatten(), pred_left_region.flatten(), pred_right_region.flatten()])

    #         label_list = [x for x in range(self.num_classes)]
    #         conf = confusion_matrix(y_true=label, y_pred=pred, labels=label_list)
    #         conf_regions.append(torch.tensor(conf))

    #     return conf_regions

    # def test_epoch_end(self, test_step_outputs):
    #     num_regions = len(self.regions)
    #     conf_total_regions = [torch.stack([output[i] for output in test_step_outputs]).sum(dim=0) for i in range(num_regions)]

    #     for region_idx, conf_total in enumerate(conf_total_regions):
    #         precision, recall, IoU = compute_results(conf_total)
    #         region_prefix = f'region_{region_idx+1}'

    #         self.log(f'{region_prefix}/average_precision', precision.mean())
    #         self.log(f'{region_prefix}/average_recall', recall.mean())
    #         self.log(f'{region_prefix}/average_IoU', IoU.mean(), prog_bar=True)

    #         for i in range(len(precision)):
    #             self.log(f"{region_prefix}(class)/precision_class_{self.label_list[i]}", precision[i])
    #             self.log(f"{region_prefix}(class)/recall_class_{self.label_list[i]}", recall[i])
    #             self.log(f'{region_prefix}(class)/Iou_{self.label_list[i]}', IoU[i])
